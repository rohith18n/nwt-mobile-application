...
import 'package:nwt_app/services/personal_assets/land_service.dart';
...
class LandPage extends StatefulWidget {
  const LandPage({super.key, this.assetId, this.isEdit});
...
class _LandPageState extends State<LandPage> {
  ...
  String selectedLandType = 'Agricultural';
  ...
  LandAssetData? _currentAssetData;
  ...
  Future<void> _fetchAssetData() async {
    ...
    try {
      final response = await _getAssetDetailsService.getLandAssetDetails(
        widget.assetId!,
      );
  ...
  void _populateFormWithAssetData() {
    ...
    areaController.text = asset.areasqft.toString();
    noteController.text = asset.notes ?? '';

    // Set dropdown selections
    selectedLandType = asset.landtype;
    selectedOwnershipType = asset.ownershiptype;

    // Set user share percentage for joint ownership
    if (selectedOwnershipType == 'Joint' && asset.usersharepercentage != null) {
      userShareholdingController.text = asset.usersharepercentage.toString();
    }
  ...
  Future<void> _submitLandData() async {
    print("submit_data for land");
    ...
    if (selectedLandType.isEmpty) {
      setState(() => _errorMessage = 'Please select land type');
      return;
    }
    ...
    try {
      ...
      if (_isEditMode && widget.assetId != null) {
        ...
        response = await LandService().updateLandPersonalAsset(
        ...
        // Create new asset
        response = await LandService().createLandPersonalAsset(
          purchasedValue: double.parse(valueController.text),
          purchasedDate: selectedDate ?? DateTime.now().toIso8601String(),
          location: locationController.text,
          areaSqFt: double.parse(areaController.text),
  ...
  double _getCurrentTotalShareholding({int? excludeIndex}) {
    ...
    for (int i = 0; i < nominees.length; i++) {
      ...
      total += (nominees[i]['sharepercentage'] as num?)?.toDouble() ?? 0;
    }
    return total;
  }
