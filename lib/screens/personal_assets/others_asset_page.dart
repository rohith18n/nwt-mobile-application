...
import 'package:nwt_app/services/personal_assets/others_asset_service.dart';
...
class OthersAssetPage extends StatefulWidget {
  const OthersAssetPage({super.key, this.assetId, this.isEdit});
...
class _OthersAssetPageState extends State<OthersAssetPage> {
  ...
  OtherAssetData? _currentAssetData;
  ...
  /// Fetches asset data for edit mode
  Future<void> _fetchAssetData() async {
    ...
    try {
      final response = await _getAssetDetailsService.getOtherAssetDetails(
        widget.assetId!,
      );
      if (response?.data != null) {
        ...
        setState(() {
          _errorMessage = 'Failed to load asset details';
        });
      }
    } catch (e) {
      setState(() {
        _errorMessage = 'Error loading asset details: ${e.toString()}';
      });
    } finally {
      setState(() {
  ...
  /// Populates form fields with fetched asset data
  void _populateFormWithAssetData(OtherAssetData assetData) {
    // Populate basic fields
    assetnameController.text = assetData.assetname ?? '';
    valueController.text = assetData.purchasedvalue.toString();
    descriptionController.text = assetData.description;
    noteController.text = assetData.notes ?? '';

    // Parse and set date
    try {
      final parsedDate = DateTime.parse(assetData.purchaseddate);
      selectedDate = parsedDate;
      selectedDateISO = parsedDate.toIso8601String();
      dateController.text = DateFormat('d-M-yyyy').format(parsedDate);
    } catch (e) {
      print('Error parsing date: ${assetData.purchaseddate}');
    }

    // Set ownership type and shareholding
    selectedOwnershipType = assetData.ownershiptype;
    if (assetData.usersharepercentage != null &&
        assetData.usersharepercentage! < 100) {
      shareholdingController.text = assetData.usersharepercentage!.toString();
    }
  ...
  Future<void> _selectDate(BuildContext context) async {
    ...
    if (picked != null) {
      setState(() {
        selectedDate = picked;
        selectedDateISO = picked.toIso8601String();
        dateController.text = "${picked.day}-${picked.month}-${picked.year}";
      });
    }
  }

  Future<void> _submitOthersAssetData() async {
    ...
    if (assetnameController.text.isEmpty) {
      setState(() => _errorMessage = 'Please enter asset name');
      return;
    }
    ...
    try {
      ...
      if (_isEditMode && widget.assetId != null) {
        ...
        // Create new asset
        response = await OthersAssetService().createOthersAssetPersonalAsset(
          assetName: assetnameController.text,
          purchasedValue: double.parse(valueController.text),
          purchasedDate: selectedDateISO!,
          description: descriptionController.text,
  ...
  /// Handles form submission with loading check
  void _handleSubmit() {
    if (_isLoading || _isLoadingAssetData) return;
    _submitOthersAssetData();
  }
                  ...
                  child: Column(
                    ...
                    children: [
                      AppInputField(
                        ...
                        hintText: "Enter asset name (e.g., Painting, Jewelry)",
                      ),
                      AppInputField(
  ...
  void _editNominee(int index) {
    _showNomineeDialog(
      ...
      initialShareholding:
          (nominees[index]['sharepercentage'] as num?)?.toString() ?? '0',
    );
  }
